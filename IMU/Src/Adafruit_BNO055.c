...
void Adafruit_BNO055_WriteData(uint8_t reg, uint8_t data)
{
	...
	HAL_I2C_Master_Transmit(GlobalConfig, _address, txdata, sizeof(txdata), 10);
}
void Adafruit_BNO055_SetAxisMap(adafruit_bno055_axis_map_t axis)
{
	...
	Adafruit_BNO055_WriteData(BNO055_AXIS_MAP_CONFIG_ADDR, axisRemap);
	Adafruit_BNO055_WriteData(BNO055_AXIS_MAP_SIGN_ADDR, axisMapSign);
}
